...
function SchemaEditor({
  ...
  return <Surface className="schema-editor" variant="raised">
    ...
    <form className="schema-editor__form" noValidate onSubmit={submit}>
      ...
      {kind !== 'index' && <div className="schema-editor__properties"><label><input checked={required} onChange={(event) => setRequired(event.target.checked)} type="checkbox" />Required</label><label><input checked={unique} disabled={type === 'files'} onChange={(event) => setUnique(event.target.checked)} type="checkbox" />Unique</label>{type === 'files' && <span className="schema-hint">A files Field cannot be unique.</span>}</div>}
      ...
      {kind !== 'index' && <details className="schema-editor__advanced"><summary><ChevronDown aria-hidden="true" size={14} />Validation, defaults, and description</summary><div className="schema-editor__grid"><FormField htmlFor="schema-edit-description" label="Description"><input id="schema-edit-description" onChange={(event) => setDescription(event.target.value)} value={description} /></FormField><FormField htmlFor="schema-edit-default" label="Default value (JSON)"><input id="schema-edit-default" onChange={(event) => setDefaultValue(event.target.value)} value={defaultValue} /></FormField><FormField htmlFor="schema-edit-validation" hint={(type === 'file' || type === 'files') ? 'File constraints: maxBytes, allowedMimeTypes, and maxFiles (files only).' : undefined} label="Validation (JSON object)"><textarea id="schema-edit-validation" onChange={(event) => setValidation(event.target.value)} rows={3} value={validation} /></FormField></div></details>}
