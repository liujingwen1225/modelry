...
- Admin 与 Application 的单条 Record `GET` 可选接受 `expand=fieldName[,fieldName...]`，最多 10 个 Applied Relation Fields；每个 Relation 最多展开 100 个目标 Record，且不递归展开。响应保留原字段中的 Record ID，并在 `_expand` 中返回目标 Record（to-one 为对象或 `null`，to-many 为数组）。未知、重复或非 Relation 字段返回 400；List 不接受 `expand`。Application API 在来源 Record View 通过后，还必须对每个目标 Record 重新评估目标 Collection 的 View Access Rule；缺失、拒绝或无法安全评估的 Relation 整体从 `_expand` 省略，不返回部分目标或额外错误。Admin 读取仍由 Control Plane 权限保护，不套用 Application Access Rule。
