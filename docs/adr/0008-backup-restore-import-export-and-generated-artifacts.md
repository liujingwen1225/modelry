...
- Runtime backup reads each snapshot-referenced object through the active File Storage Provider built from the persisted Project configuration and Project Secret provider. The CLI follows the same Provider construction path while the Runtime is stopped; it never assumes Local storage and never falls back to Local when the configured Provider or its credentials are unavailable.
...
- Preflight validates the format name and version, that every archive entry listed in the manifest is present with a matching digest, that no unlisted entry exists, that the database payload opens and exposes Modelry's internal migration table, and that the database format is not newer than the Runtime. The manifest is authoritative metadata and its payload integrity is validated; without a trusted outer digest or signature, the bundle does not claim authenticity or detection of malicious rewrites to structurally valid metadata such as counts, creation time, or Runtime version. Preflight reports the project id, Runtime version, dates, counts, and any incompatibility as structured findings.
- Apply refuses to run while another process holds the project runtime lock, and refuses to replace an existing project unless the operator passes `--force`. When it does run, it extracts into a staging directory inside the managed directory, re-validates the staged database, and only then replaces the database and referenced object keys. Write-ahead journal intents are synced before the actions they describe. The replacement has three phases: new content is prepared first, originals are moved to same-directory backups second, and everything is activated last. On POSIX, each created destination directory and each destination or journal/marker namespace change is followed by a sync of its containing parent directory; failures are returned. Staged files are synced before activation. A separately written, flushed, and synced commit marker is atomically renamed into place, then the managed directory is fsynced. That final directory sync is the single commit point: before it, errors roll back to the byte-identical old state; after it, recovery only cleans backups and never rolls back the new state. Rollback only deletes a destination when the journal's backup file actually exists; otherwise the destination is still the original and is left untouched. Replacing the database also removes its `-wal`/`-shm` sidecars so a stale log cannot be replayed into the new file. Windows uses file Sync and same-volume atomic rename because Go's standard library does not expose POSIX directory fsync; Windows directory-entry durability follows the filesystem's guarantees and is not claimed equivalent to POSIX. Restore fails closed on platforms without an explicit directory-durability implementation.
- Restore replaces the logical project state described by the bundle; it does not physically mirror or empty the target Provider. Unreferenced objects already at the target may remain temporarily and follow the existing File Storage orphan-reconcile grace period. A Runtime refuses to start while an uncommitted journal exists; committed journals are cleanup-only, and interrupted uncommitted journals are rolled back before the "already contains state" check.
- A pre-marker legacy journal that ends in `journalDone` is inherently ambiguous because the old implementation could expose the row before its file sync succeeded. The new Runtime and ordinary restore fail closed instead of treating that row as committed. After making a full project-directory copy and reviewing the active files, an operator can explicitly choose `modelry restore --force --resolve-legacy-restore=accept-current --from <bundle>`; this keeps current destinations, removes legacy backup artifacts, and applies the supplied bundle. Recovering a desired old state requires restoring it from the saved directory copy. New-format journals always use the durable marker and do not need this option.
