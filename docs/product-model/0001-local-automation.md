...
- A **Webhook Delivery** is the durable, immutable attempt intent produced by an Event Hook, Job, or Owner-requested test. It has one stable Delivery ID and idempotency key across automatic retries and manual redrives. Record Event requests also repeat the Event ID in `X-Modelry-Event-Id`. An Owner-requested synthetic test is the only send allowed while its Webhook is disabled.
